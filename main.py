...
def write_and_run_temp_python(code: str, injected_pickle: str = None, timeout: int = 60) -> Dict[str, Any]:
    ...
    # Add DuckDB support for S3 queries
    preamble.extend([
        "import duckdb",
        "import os",
...
4. Your Python code will run in a sandbox with:
   ...
   - DuckDB with httpfs and parquet extensions loaded, S3 region set to 'ap-south-1'
   - A connection object called `conn` for DuckDB queries
   - A helper function `plot_to_base64(max_bytes=100000)` for generating base64-encoded images under 100KB.
5. For S3 data access, use DuckDB queries like: conn.execute("SELECT * FROM read_parquet('s3://bucket/path/*.parquet')").df()
6. When returning plots, always use `plot_to_base64()` to keep image sizes small.
7. Make sure all variables are defined before use, and the code can run without any undefined references.
8. For the Indian High Court dataset, use the S3 path: 's3://indian-high-court-judgments/metadata/parquet/year=*/court=*/bench=*/metadata.parquet'
"""),
    ("human", "{input}"),
